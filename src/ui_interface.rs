...
lazy_static::lazy_static! {
    ...
    static ref TEST_ASYNC_JOB_STATUS : Arc<Mutex<String>> = Arc::new(Mutex::new(" ".to_string()));
...
pub fn post_request(url: String, body: String, header: String) {
    *ASYNC_JOB_STATUS.lock().unwrap() = " ".to_string();
    std::thread::spawn(move || {
        let result = match crate::post_request_sync(url.clone(), body, &header) {
            Err(err) => err.to_string(),
            Ok(text) => text,
        };
        log::info!("POST Response from Node.js ({}): {}", url, result);
        *ASYNC_JOB_STATUS.lock().unwrap() = result;
    });
}
...
pub fn get_request(url: String, header: String) {
    {
        let mut status = ASYNC_JOB_STATUS.lock().unwrap();
        *status = " ".to_owned();
    } // Mutex tự động unlock ở đây
    
    std::thread::spawn(move || {
        let result = match crate::http_request_sync(url.clone(), "GET".to_owned(), None, header) {
            Err(err) => err.to_string(),
            Ok(text) => text,
        };
        log::info!("GET Response from Node.js ({}): {}", url, result);
        let mut status = ASYNC_JOB_STATUS.lock().unwrap();
        *status = result;
    });
}
...
#[inline]
#[cfg(not(feature = "flutter"))]
pub fn test_get_request(url: String) {
    // Reset trạng thái về rỗng hoặc hiển thị đang xử lý
    if let Ok(mut status) = TEST_ASYNC_JOB_STATUS.lock() {
        *status = "Processing...".to_string();
    }

    std::thread::spawn(move || {
        // Sử dụng hàm có sẵn của dự án để gọi request
        let result = match crate::http_request_sync(url.clone(), "GET".to_owned(), None, "".to_owned()) {
            Err(err) => format!("Error: {}", err),
            Ok(text) => {
                // Xử lý dữ liệu JSON nhận được từ Node.js
                // Thử parse chuỗi text thành struct ApiResponse
                match serde_json::from_str::<ApiResponse>(&text) {
                    Ok(data) => data.message, // Chỉ lấy nội dung tin nhắn nếu parse thành công
                    Err(_) => text,           // Nếu không phải JSON, trả về text thô (phòng hờ)
                }
            }
        };

        log::info!("TEST GET Response from Node.js ({}): {}", url, result);
        
        // Cập nhật kết quả cuối cùng vào biến toàn cục
        if let Ok(mut status) = TEST_ASYNC_JOB_STATUS.lock() {
            *status = result;
        }
    });
...
#[inline]
pub fn get_test_async_job_status() -> String {
    TEST_ASYNC_JOB_STATUS.lock().unwrap().clone()
}
